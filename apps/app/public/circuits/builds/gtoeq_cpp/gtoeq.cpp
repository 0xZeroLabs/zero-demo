...
void GreaterThanOrEqual_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void GreaterThanOrEqual_3_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[4] = { 
Num2Bits_0_run,
LessThan_1_run,
GreaterEqThan_2_run,
GreaterThanOrEqual_3_run };
...
uint get_size_of_witness() {return 20;}
...
ctx->componentMemory[coffset].templateName = "GreaterThanOrEqual";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
GreaterThanOrEqual_3_create(1,0,ctx,"main",0);
GreaterThanOrEqual_3_run(0,ctx);
}
